  ...
  $('.parallax').parallax();
   ...
   //Uncomment re-initializing the cookies session
   //localStorage.removeItem('cookies_box');
      
   function cookies_setup(){

    if(!localStorage.getItem('cookies_box')){
      TweenMax.fromTo("#section_cookies", 0.4, {opacity:0, y:20}, {opacity:1, y:0, delay:3});
    }else{
      remeveCookiesBox();
    }

    function remeveCookiesBox(){
      $("#section_cookies").delay(0.8).remove();
    }
    
    $("#section_cookies .close").click(function(){
      localStorage.setItem("cookies_box", "1");
      TweenMax.fromTo("#section_cookies", 0.4, {opacity:1, y:0}, {opacity:0, y:0, scale:0.9, delay:0, onComplete:remeveCookiesBox});
      return false;
      //alert(localStorage.getItem('cookies_box'))
    })
  ...
  if($("#audioLP").length){
    ...
    initBGsong = false;
    $("#audioBox i.off").click(function(){
      $(this).hide();
      $("#audioBox i.on").show();
      audioLP.play(); 
    })
    $("#audioBox i.on").click(function(){
      $(this).hide();
      $("#audioBox i.off").show();
      audioLP.pause(); 
    })  
...
function showloaderForm(form, options){
  // $(".loaderForm").css({"display":"inline-block"});
  // $(".arrowSub").css({"display":"none"});
  //alert("loader ON")
}

function hideLoaderForm(form, options){
      // $(".loaderForm").css({"display":"none"});
      // $(".arrowSub").css({"display":"inline-block"});
      //alert("loader OFF")
}
...
function ajaxValidationCallback(status, form, json, options){
  ...
  if (window.console)  
    console.log(status);
  // Mettre ici le code pour le chargement
  if (status === true) {    
      //alert(json.whatsthestate)
    if(json.whatsthestate == 1){
      //$("#message_ok").fadeTo(600,1);
      /*showAlert("Tribal DDB vous remercie pour votre enregistrement, un email de confirmation vous sera envoyé sur votre messagerie électronique.");*/
      //alert("done.")
        hideLoaderForm();
        //alert(json.userName)
        $("#msg_form_action").html(json.msg).fadeTo(600,1);
        $("#msg_callback").html(json.msg).fadeTo(600,1);
        form[0].reset();
        //alert("Everything is good "+json.whatsthestate)   
        window.location = json.mediaLink;    
        if(json.mediaLink != null){
          //$("#downloadLink_beat").attr("href",json.mediaLink)
          //$("#downloadLink_beat").trigger("click")
          forceDownload(json.mediaLink)
        }
    }else if(json.whatsthestate == "notdone"){
      /*showAlert("Votre enregistrement a déjà été pris en compte.");*/
        //alert("Not good :(");
        hideLoaderForm();
    }else{
...
if($(".generic_form").length){
  jQuery(".generic_form").validationEngine("attach",{
    ...
    onBeforeAjaxFormValidation : showloaderForm
  })
}
